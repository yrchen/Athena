...
out_song()
{
  ...
  move(b_lines - 2, 0);
  prints("\033[1;3%dm%s\033[m  籌碼還有 %d 元", time(0) % 7, msg[count], cuser.silvermoney);
  clrtoeol();
  if (++count == 9)
    count = 0;
}
...
main_bingo()
{
  ...
  while (1)
  {
    ...
    getdata(2, 0, "請問要下注多少呢？(1 ~ 50000) ", buf, 6, DOECHO, "");
    money = atoi(buf);
    if (money < 1 || money > 50000 || money > cuser.silvermoney)
      break;				/* 離開賭場 */
	...
	if (!used[i - 1])
      ...
      do
      {
        getdata(b_lines - 4, 0, "請輸入您的號碼：", buf, 3, DOECHO, "");
        i = atoi(buf);
    ...
    if (success)
    {
      ...
      inmoney(money);
      game_log("BINGO", "贏了 %d 元", money);
    }
    else
    {      
      strcpy(buf, "運氣不佳...再來一盤吧！");
      game_log("BINGO", "輸了 %d 元", money);
    }
    pressanykey(buf);
