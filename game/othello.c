/* $Id: othello.c,v 1.1 2000/05/08 12:26:36 davidyu Exp $ */
#include "bbs.h"

#define LOGFILE "log/othello.log"
#define SECRET "log/othello.secret"
#define NR_TABLE 2
...
static int choose() {
    ...
    do 
    {
	getdata(4, 0, "請選擇一個對象和您對打:(1~3)", thinkstep, 2,LCECHO,0);
    } while((thinkstep[0] < '1' || thinkstep[0] > '3') && thinkstep[0] != 'q');
    clear();
    switch(thinkstep[0]) 
    {
    case 'q':
