...
userec record;
char myfrom[128], mysub[128], myname[128], mypasswd[128], myboard[128], mytitle[128], mydigest[128];
  ...
  userec *record;
{
  ...
  {
    lseek(fh, (off_t)((usrnum - 1) * sizeof(userec)), SEEK_SET);
...
justify_user()
{
  ...
  strncpy(record.justify, myfrom, REGLEN + 1);
  /* 雙重認證,email通過後只能擁有post權限 */
  /* wildcat 980909			  */
  record.userlevel |= (PERM_POST) ;
  sprintf(buf1,BBSHOME"/home/%s", record.userid);
  stampfile(buf1, &mhdr);                                              
  strcpy(mhdr.owner, "站長");                                      
  sprintf(mhdr.title, "[EMail認證通過]");
  mhdr.savemode = 0;
  unlink(buf1);
  f_cp("etc/replyemail", buf1,O_TRUNC);
  sprintf(buf1,BBSHOME"/home/%s/.DIR",record.userid);
  rec_add(buf1, &mhdr, sizeof(mhdr));
}
  ...
  char *magic;
{
  ...
  if (ptr = (char *) strchr(magic, '('))
  {
	...
	*ptr++ = '\0';
	...
	if (!strcasecmp(magic, record.userid))
	{
...
post_article()
{
  ...
  rec_add(index, &header, sizeof(header));
...
digest_article()
{
  ...
  if (ptr = (char *) strstr(index, myname))
  {
	...
	if (fh = fopen(index, "a+"))
	{
	  ...
	  fh = fopen(index, "a+");
	  fprintf(fh, "Name=◇ %s\nDate=%02d/%02d/%02d\nPath=%s\n\n",
	    mytitle, ptime->tm_mon + 1, ptime->tm_mday, ptime->tm_year % 100, mydigest);
	  fclose(fh);
	  return 0;
	}
      }
  ...
  while (!*myname || !*mypasswd || !*myboard || !*mytitle)
  {
	...
	*token = '\0';
	...
	strcpy(myname, token);
      ...
      else if (!strcasecmp(key, "passwd") || !strcasecmp(key, "password") || !strcasecmp(key, "passward"))
      {
	...
	strcpy(myboard, token);
      ...
      else if (!strcasecmp(key, "title") || !strcasecmp(key, "subject"))
      {
	...
	*ip = '\0';
    ...
    else if (!strncmp(mybuf, "Subject: ", 9))
    {
      ...
      {
	verify_user(ptr + strlen(TAG_VALID));
	return -1;
      }
      if (ptr = strstr(mybuf, "[WD SYSTEM]To "))
      {
	verify_user(ptr + 14);
	return -1;
      }
