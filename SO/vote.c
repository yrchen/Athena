...
static char STR_bv_comments[] = ".comments";	/* §ë²¼ªÌªº«ØÄ³ */
...
//  if (stat(STR_fn_vote_polling, &st) == -1/* || st.st_mtime > (now - 3600)*/)
  if(!dashf(STR_fn_vote_polling))
  {
    pressanykey("No Such File : %s !", STR_fn_vote_polling);
    return;
  }
...
time_t
next_closetime(char *direct, time_t closetime)
{
  int num;
  fileheader vfh;
...
void
delete_vote_files(int ent, fileheader *fhdr, char *direct, char *fpath)
{
  int len;
...
void
result_post(fileheader *fhdr, char *bname, char *fpath)
{
...
void								    /* ¤H¬°¤¤¤î */
make_results(fileheader *fhdr, int *counts, char *buf, char *fpath, int interrupt)
{
  FILE *fout, *fin;
  int len, total = 0, num;
...
int
b_close(boardheader *bh)
{
  char buf[STRLEN], fpath[STRLEN], inchar[3];
...
int
vote_flag(fileheader *fhdr, char val)
{
  char buf[256], flag;
  int fd, num, size;
...
int
same(int compare, int list[], int num, int rep)
{
  int n;
  int tmp = 0;
...
static int
do_vote(int ent, fileheader *fhdr, char *direct)
{
  FILE *fp;
      ...
      *vote = tolower(*vote);
      ...
      else
      {
        ...
        else
        {
          ...
          substitute_record(fn_passwd, &cuser, sizeof(userec), cuser.uid); /* °O¿ý */
...
static int
maintain_vote(int ent, fileheader *fhdr, char *direct)
{
  char buf[STRLEN], fpath[STRLEN], inbuf[STRLEN], inchar[3];
  int len, page = 0;
  FILE *fp;
...
struct one_key vote_comms[] = {
  'r',         do_vote,       0, "§ë²¼",0,
  Ctrl('P'), make_vote, PERM_BM, "Á|¿ì§ë²¼",0,
  'M',   maintain_vote, PERM_BM, "§ë²¼ºÞ²z",0,
  '\0', NULL, 0, NULL,0};


void 
votedoent(int num, fileheader *ent, int row, char *bar_color)
{
  char closeday[9];
  struct tm *ptime;
  time_t dtime;
  ...
  prints("%4d %c %-5s%-4.3s %-11s %s[%s] [m%s %s \033[m", 
    num, vote_flag(ent, 0) ? ' ':'+', ent->date, ent->title + 65, closeday,
...
votetitle(int tag)
{
  switch(tag)
  {
    case RC_FULL:
      ...
      outs("\
ªO¥D±M¥ÎÁä: [^P]Á|¿ì¤@¦¸§ë²¼ [M]¨ú®ø§ë²¼/´£¦­¶}²¼\n\
" COLOR3 "½s¸¹   ¶}©l¤é´Á  µ²§ô¤é´Á    §ë  ²¼  ¥D  ÃD                                    \033[0m");
      break;
    case RC_FOOT:
      move(b_lines, 0);
      prints("\033[0;44m  §ë ²¼ ©Ò  \033[m%s%67.67s\033[m", COLOR3,
        "¡ö¡ô¡õ¡÷)¾ÉÄý  Enter)¶}©l§ë²¼  Q)Â÷¶}  ");
      break;
  }
}
...
ShowVote(char *bname)
{
  ...
  i_read(VOTING, buf, votetitle, votedoent, vote_comms, Ctrl('P'));
...
*/
    ...
    do
    {
      if(redraw)
      {
        ...
        show_file("etc/votetitle",1,6,ONLY_COLOR);
        move(7, 0);
        prints("%s  ½s¸¹  ¬Ýª©¦WºÙ         ¤¤  ¤å  ±Ô  ­z                             §ë²¼¼Æ¶q   \033[m", COLOR3);
        redraw=0;
      }
...
//        vote_flag(vdata[i+pagenum*pageboard].bname,'\0')?"[1;36m¤w§ë[0m":"[1;31m¥¼§ë[0m");
      ...
      move(b_lines,0);
      prints("\033[0;44m  §ë²¼¿ï³æ  [0m%s%67.67s\033[m", COLOR3, 
        "¡ö¡ô¡õ¡÷|PgUp|PgDn)¾ÉÄý  Enter)§ë²¼  Q)Â÷¶}  ");
      move(b_lines,79);
      ch=igetkey();
      strcpy(vdata[pass].pointer,"¡@");
      switch(ch)
