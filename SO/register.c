...
new_register(int mode)
{
  ...
  move(b_lines - 5, 0);
  outs(" \
½Ð¿ï¾Ü¡G1) §ÚÃÙ¦P¥»¯¸¬[¯¸ªº²z©À¡AÄ@·NÄ~Äò¬ÛÃöµù¥Uµ{§Ç¡I\n \
        2) §Ú¤£ÃÙ¦P¥»¯¸²z©À¡A²{¦b°¨¤WÂ÷¶}¡I\n \
        3) §Ú¥ý¥Î guest °ÑÆ[¤@¤U¦A°µ¨M©w¡I");

  getdata(b_lines - 1, 0, "§Úªº¨M©w¬O¡G[2]", genbuf, 3, DOECHO, 0);
  switch(genbuf[0])
    {
     case '1':
       break;
     case '3':
       pressanykey("½Ð­«·s¥Î guest µn¤J :)");
       oflush();
       exit(1);
     default:
       pressanykey("ÁÂÁÂ°ÑÆ[ :)");
  ...
  if(getans2(b_lines, 0, "½Ð°Ý¬O§_±µ¨ü¥»¯¸ªºÁô¨pÅv«OÅ@¬Fµ¦ ", 0, 2, 'n') != 'y')
  {
    pressanykey("ÁÂÁÂ°ÑÆ[ :)");
    oflush();
    exit(1);
...
int u_verify()
{
  ...
  {
    pressanykey("±z¤w¸g³q¹L»{ÃÒ¡A¤£¶·­n¿é¤J MagicKey¡I");
    return XEASY;
  }
  ...
  if (!dashf(keyfile))
  {
    if(win_select("»{ÃÒ½X", "±zÁÙ¥¼µo»{ÃÒ«H¡A­nµo¥X¶Ü¡H", 0, 2, 'y') == 'y')
      mail_justify(cuser);
  ...
  {
    pressanykey("¶}±ÒÀÉ®×¦³°ÝÃD¡A½Ð³qª¾¯¸ªø¡I");
    fclose(fp);
    return XEASY;
  }
  ...
  {
    if (strcmp(key, inbuf))
      pressanykey("¿ù»~¡A½Ð­«·s¿é¤J¡I");
    else
    {
      int unum = getuser(cuser.userid);
      
      unlink(keyfile);
      pressanykey("®¥³ß±z³q¹L»{ÃÒ¡AÅwªï¥[¤J :)");
      cuser.userlevel |= (PERM_PAGE | PERM_POST | PERM_CHAT | PERM_LOGINOK);
      mail2user(cuser.userid, "[µù¥U¦¨¥\\Åo]", "etc/registered", 0);
