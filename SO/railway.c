...
#define HTTP_PORT	80
#define SERVER_railway	"203.67.46.20"
...
site_show()
{
  ...
  move(b_lines - 2, 0);
  outs("查詢車種：1)對號快車 2)非對號車       1)直達 2)轉車");
...
time_query()
{
  ...
  if (!getdata(b_lines - 3, 0, "查詢時間：", fr_tm, 3, DOECHO, ""))
  {
    strcpy(fr_tm, "0");
    move(b_lines - 3, 10);
    outs("00");
  ...
  if (!getdata(b_lines - 3, 16, "至 ", to_tm, 3, DOECHO, ""))
  {
    strcpy(to_tm, "24");
    move(b_lines - 3, 19);
    outs("24");
  ...
  if (!getdata(b_lines - 3, 25, "之間 1)開車 2)到達：[1] ", ans, 3, DOECHO, "") || ans[0] != '2')
  {
    strcpy(ttime, "start");
    move(b_lines - 3, 37);
    clrtoeol();    
...
rail_query()
{
  ...
  if (!getdata(b_lines - 2, 0, "查詢車種：1)對號快車 2)非對號車：[1] ", ans, 3, LCECHO, "") || ans[0] != '2')
  {
    strcpy(typer, "fast");
    move(b_lines - 2, 21);
    clrtoeol();
  ...
  refresh();	/* 再次 getdata() 會出現前面的控制碼，要 refresh */

  if (!getdata(b_lines - 2, 38, "1)直達 2)轉車：[1] ", ans, 3, DOECHO, "") || ans[0] != '2')
  {
  ...
  int undirect;		/* 1: 轉車 */
{
  ...
  while (str_cmp(tag, "/html"))
  {
	...
	if (undirect)
	{
  	...
  	fprintf(fpw, "\n"
	  "┌───┬──┬─┬───┬─┬───┬───┬───┬──┬───────┐\n"
	  "│ 乘坐 │車次│山│ 發車 │開│ 終點 │ 啟程 │ 到達 │全票│補          充│\n"
	  "│ 車種 │編號│海│ 車站 │往│ 車站 │ 時間 │ 時間 │價錢│備          註│");
      }
  ...
  char *s;
{
  ...
  {
    if (getans2(b_lines, 0, "您要將查詢結果寄回信箱嗎？ ", 0, 2, 'n') == 'y')
      mail2user(cuser.userid, "[備 忘 錄] 火車時刻查詢結果", dst, FILE_READ);
  }
  unlink(dst);
  unlink(src);
...
main_railway()
{
  ...
  ch = answer("查詢 0)今天 1)明天 2)後天 3456789)天後的時刻表？[Q] ") - '0';
  if (ch < 0 || ch > 9)
  {
