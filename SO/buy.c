...
p_cloak()
{
  if (answer(currutmp->invisible ? "½T©w­n²{¨­?[y/N]" : "½T©w­nÁô¨­?[y/N]") != 'y')
    return;
...
p_fcloak()
{
  ...
  if (answer("½T©w­nªá $500 ¾Ç²ß²×·¥Áô¨­¤jªk¡H[y/N]") != 'y')
    return;
...
p_ffrom()
{
  ...
  if (answer("½T©w­nªá $1000 ÁÊ¶R­×§ï¬G¶mÄ_¨å¡H[y/N]") != 'y')
     return;
  rec_get(fn_passwd, &xuser, sizeof(xuser), usernum);
...
p_ulmail()
{
  ...
  if (answer("½T©w­nªá $100000 ÁÊ¶RµL¤W­­«H½c?[y/N]") != 'y')
    return;
  rec_get(fn_passwd, &xuser, sizeof(xuser), usernum);
