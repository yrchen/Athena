...
int m_newbrd()
{
  ...
  getdata (8, 0, "¬ÝªO¥DÃD¡G", genbuf, BTLEN-7, DOECHO, 0);
  if (genbuf[0])
  ...
  if (!(newboard.brdattr & BRD_GROUPBOARD || newboard.brdattr & BRD_CLASS))
  {
      ...
      if (getans2(12, 0, "¬O§_¬°¹ÎÅéªO¡H", 0, 2, 'n') == 'y')
        newboard.brdattr |= BRD_GROUP;
#endif

      getdata (12, 0, "¬ÝªOÄÝ©Ê¡GP)­Ó¤H  G)¹ÎÅé  N)¤@¯ë¡H[N] " ,ans, 4, LCECHO, "N");
      if (*ans == 'p')
      {
        newboard.brdattr |= BRD_PERSONAL;
      }
...
m_mod_board (char *bname)
{
  ...
  bperm_msg (&bh);
  prints ("%s³]­­\n"
	  "¥izap:%s ²Î­p:%s Âà«H:%s ¸s²Õ:%s ¨p¤H:%s ÁôÂÃ:%s °Î¦W:%s Àu¨}:%s ­Ó¤H:%s ¹ÎÅé:%s",
	  bh.level ? "¦³" : "¤£",
  ...
  getdata (9, 0, "¬ÝªO¡GD)§R°£  E)³]©w  L)¤W­­  B)ªO¥D  C)´y­z  Q)¨ú®ø¡H[Q] ", genbuf, 3, LCECHO, 0);
  ...
  switch (*genbuf)
  {
	...
	strncpy (newbh.title, genbuf, 4);
      ...
      getdata (14, 0, "¬ÝªO¥DÃD¡G", genbuf, BTLEN-7, DOECHO, bh.title + 7);
      if (genbuf[0])
...
//        newbh.totaltime = atol(genbuf);
	...
	if (getans2(17, 0, "¥i¥H°Î¦W¡H", 0, 2, newbh.brdattr & BRD_ANONYMOUS ? 'y' : 'n') == 'y')
	  ...
	  char ans[4];
          ...
          if (getans2(18, 0, "¬O§_¬°®a±ÚªO¡H", 0, 2, bh.brdattr & BRD_GROUP ? 'y' : 'n') == 'y')
	  ...
	  if (HAS_PERM (PERM_SYSOP) && !(newbh.brdattr & BRD_HIDE))
	  {
	    ...
	    {
	      getdata (22, 0, "­­¨î¡GR)¾\\Åª  P)µoªí¡H ", genbuf, 4, LCECHO,
