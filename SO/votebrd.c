...
do_voteboardreply(fileheader *fhdr)
{
   ...
   {
     pressanykey("¤W¯¸®É¼Æ¤£¨¬¡A¤£±o°Ñ»P³s¸p¡I");
     return;
   }
   ...
   while(fgets(genbuf, sizeof(genbuf), fp))
   {
      ...
      else if (!strncmp(genbuf+4, cuser.userid, len))
      {
         ...
         opnion[0] = 'n';
         if (getans2(7, 0, "­n­×§ï±z¤§«eªº³s¸p¶Ü¡H", 0, 2, 'n') != 'y')
         {
            fclose(fp);
            return;
         }
...
*/      
   ...
   while(fo)
   {
      ...
      genbuf[j] = '\0';
      i++;
      if (!strncmp("----------", genbuf, 10))  //Åª¨ì¤À¹j½u¸õ¥X
         break;
      if (i > 3)
         prints(genbuf);
      fprintf(fo, "%s", genbuf);
...
//   fprintf(fo, "%s", genbuf);  //¤À¹j½u¼g¤J, ²¾¨ì¤U­±

   do
   {
      char *choose[3]={"yY)¤ä«ù","nN)¤Ï¹ï","qQ)¨ú®ø"};
      
      clear();
      
      if((*opnion = getans2(18, 0, "½Ð°Ý±z¡G", choose, 3, 'q')) == 'q')
      {
         flock(fd, LOCK_UN);
         close(fd);
         fclose(fo);
...
*/
      fprintf(fo, "%3d.%-15s: [1;37;40m%s\033[m\n", i, cuser.userid, reason);
   }
   i = 0;
   fprintf(fo, "%s", genbuf);
...
do_voteboard(void)
{
    ...
    do
    {
      getdata(3, 0, "½Ð¿é¤J³s¸pÃþ§O¡G", topic, 3, DOECHO,0);
      temp = atoi(topic);
    }while(temp <= 0 && temp >= 9);
    ...
    case 1:
       ...
       strcat(genbuf, topic);
       strcat(genbuf, "\n¬O§_ÁôÂÃ: ");
       getdata(8, 0, "¬O§_ÁôÂÃ  Y)½T©w  N)¨ú®ø¡H¡G", topic, 3, DOECHO,0);
       strcat(genbuf, topic);	                                  
       strcat(genbuf, "\n¥Ó½Ð­ì¦]: \n");
       move(9,0);
       outs("½Ð¿é¤J¥Ó½Ð­ì¦] (¦Ü¦h¤Q¦æ)¡A­n²M·¡¶ñ¼g¤£µM¤£·|®Ö­ã³á¡I");
       for(i= 9;i<19;i++)
       {
    ...
    case 2:
       ...
       sprintf(genbuf, "%s\n\n%s%s\n","¼o°£ÂÂªO", "­^¤å¦WºÙ: ", topic);
       strcat(genbuf, "\n¼o°£­ì¦]: \n");
       outs("½Ð¿é¤J¼o°£­ì¦] (¦Ü¦h¤­¦æ)¡A­n²M·¡¶ñ¼g¤£µM¤£·|®Ö­ã³á¡I");
       for(i= 8;i<13;i++)
       {
          if (!getdata(i, 0, "¡G", topic, 60, DOECHO,0))
             break;
    ...
    case 3:
       ...
       strcat(genbuf, "\n¥Ó½Ð¬F¨£: \n");
       outs("½Ð¿é¤J¥Ó½Ð¬F¨£ (¦Ü¦h¤­¦æ)¡A­n²M·¡¶ñ¼g¤£µM¤£·|®Ö­ã³á¡I");
       for(i= 9;i<14;i++)
       {
          if (!getdata(i, 0, "¡G", topic, 60, DOECHO,0))
             break;
    ...
    case 4:
       ...
       do
       {
         if (!getdata(6, 0, "½Ð¿é¤JªO¥DID¡G", topic, IDLEN + 1, DOECHO,0))
            return RC_FULL;
         else if (!userid_is_BM(topic, bcache[i].BM))
            outs("¤£¬O¸ÓªOªºªO¥D");
         else
            break;
       }
       ...
       strcat(genbuf, topic);
       strcat(genbuf, "\n½}§K­ì¦]: \n");
       outs("½Ð¿é¤J½}§K­ì¦] (¦Ü¦h¤­¦æ)¡A­n²M·¡¶ñ¼g¤£µM¤£·|®Ö­ã³á¡I");
       for(i= 8;i<13;i++){
          if (!getdata(i, 0, "¡G", topic, 60, DOECHO,0))
             break;
          strcat(genbuf, topic);
    ...
    case 6:
       ...
       if (!getdata(6, 0, "½Ð¿é¤J¤p²Õªø ID¡G", topic, IDLEN + 1, DOECHO,0))
          return RC_FULL;
       strcat(genbuf, topic);
       strcat(genbuf, "\n½}§K­ì¦]: \n");
    ...
    case 7:
       ...
       sprintf(genbuf, "%s\n\n%s%s\n","¯¸¥Á¤½§ë", "¤½§ë¥DÃD: ", topic);
       strcat(genbuf, "\n¤½§ë­ì¦]: \n");
       outs("½Ð¿é¤J¤½§ë­ì¦] (¦Ü¦h¤Q¦æ)¡A­n²M·¡¶ñ¼g¤£µM¤£·|®Ö­ã³á");
       for(i= 8;i<18;i++)
       {
          if (!getdata(i, 0, "¡G", topic, 60, DOECHO,0))
             break;
