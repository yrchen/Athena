...
extern struct BCACHE *brdshm;	//by SugarII for «ös ¶iºëµØ°Ï
...
/*
   woju
   getdata(b_lines - 1, 0, "¤w¶W¹L¤@¬P´Á¨S§ó·s¡A¬O§_§ó·s¸ê®Æ  Y)½T©w  N)¨ú®ø¡H[Y] ",
   buf, 4, LCECHO, 0);
   if (*buf != 'n')
 */
      return fopen (path, "w");
...
g_searchtitle (GMENU * pm, int rev)
{
  ...
  int pos;
  if (getdata (b_lines - 1, 1, "[·j´M]ÃöÁä¦r¡G", search_str, 40,
	       DOECHO, 0))
    if (!*search_str)
      return pm->now;
...
a_showmenu (AMENU *pm)
{
  ...
  move(1, 0);
  clrtoeol();
  prints("%s\033[30m¢«\033[37m¬ÝªO¢@³Ì·R¢@¤å³¹¢¨%sºëµØ°Ï\033[m¢©                                        ^Z)§Ö³t¿ï³æ",
  	 COLOR1, 
  	 COLOR3);
     ...
     AMENU *pm;
{
  ...
  {
    if(answer("¦¹ÀÉ®×/¥Ø¿ý¤£¥i§R°£ , ¬O§_­n§R°£¯Á¤Þ  Y)½T©w  N)¨ú®ø¡H")=='y')
    {
  ...
  if (pm->header[pm->now - pm->page].filename[0] == 'H' &&
    ...
    {
      if (getans2(b_lines - 1, 1, "±z½T©w­n§R°£¦¹ºëµØ°Ï³s½u¶Ü¡H", 0, 2, 'n') != 'y')
	return;
      if (rec_del (buf, FHSZ, pm->now + 1) == -1)
	return;
    }
    ...
    {
      if (getans2(b_lines - 1, 1,"±z½T©w­n§R°£¦¹ symbolic link ¶Ü¡H", 0, 2, 'n') != 'y')
	return;
      if (rec_del (buf, FHSZ, pm->now + 1) == -1)
	return;
      unlink (fpath);
    ...
    {
      if (getans2(b_lines - 1, 1, "±z½T©w­n§R°£¦¹ÀÉ®×¶Ü¡H", 0, 2, 'n') != 'y')
	return;
      if (rec_del (buf, FHSZ, pm->now + 1) == -1)
	return;
...
*/
    ...
    {
      if (getans2(b_lines - 1, 1, "±z½T©w­n§R°£¾ã­Ó¥Ø¿ý¶Ü¡H", 0, 2, 'n') != 'y')
	return;
      if (rec_del (buf, FHSZ, pm->now + 1) == -1)
	return;
/*
...
*/
    ...
    /* Ptt ·l·´ªº¶µ¥Ø */
    {
      if (getans2(b_lines - 1, 1, "±z½T©w­n§R°£¦¹·l·´ªº¶µ¥Ø¶Ü¡H", 0, 2, 'n') != 'y')
	return;
      if (rec_del (buf, FHSZ, pm->now + 1) == -1)
	return;
    }
     ...
     AMENU *pm;
{
  ...
  if (copyfile[0])
  {
    ...
    sprintf (buf, "½T©w­n«þ¨© [%s] ¶Ü¡H", copytitle);
...
a_appenditem (AMENU *pm)
{
  ...
  if (copyfile[0])
      ...
      if (dashf (copyfile))
	{
	  ...
	  if (dashf (fname))
	    {
	      sprintf (buf, "½T©w­n±N[%s]ªþ¥[©ó¦¹¶Ü  Y)½T©w  N)¨ú®ø¡H[N] ", copytitle);
	      ...
	      if (ans[0] == 'y')
		{
		  if ((fp = fopen (fname, "a+")) != NULL)
		      ...
		      if ((fin = fopen (copyfile, "r")) != NULL)
			{
			  ...
			  buf[74] = '\0';
			  fprintf (fp, "\n> %s <\n\n", buf);
			  getdata (b_lines - 1, 1, "¬O§_¦¬¿ýÃ±¦WÀÉ³¡¥÷  Y)½T©w  N)¨ú®ø¡H[Y] ", ans, 3, LCECHO, 0);
     ...
     int rev;
{
  ...
  getdata (b_lines - 1, 1, "[·j´M]ÃöÁä¦r¡G", search_str, 40, DOECHO, 0);
     ...
     AMENU *pm;
{
  ...
  if (dashl (buf))
    ...
    pressanykey ("¦¹¶µ¥Ø¤w·l·´¡A«ØÄ³±N¨ä§R°£¡I");
}
...
a_newitem (AMENU *pm, int mode)
{
  ...
  switch (mode)
  {
    ...
    case ADDGOPHER:
      ...
      if (!getdata (b_lines - 2, 1, "¿é¤J URL ¦ì§}¡G", item.filename + 2, 61, DOECHO, 0))
	return;
      strcpy (item.title, "¡¹ ");	/* A1BB */
      break;
	  ...
	  if (HAS_PERM (PERM_LOGINOK))
		...
		sethomefile (paste_fname, cuser.userid, "paste_path");
		  ...
		  fprintf (fp, "%.*s\n%.*s\n%d\n",
			   ...
			   paste_level);
		  fclose (fp);
		  pressanykey ("¥Ø«e Paste ¥Ø¿ý¬° %s", paste_path);
	      }
	    }
	  else
	    bell ();
...
//		  strcpy (item.title, fhdr->title + ((currutmp->pager > 1) ? 3 : 0));
	...
	case Ctrl ('S'):
	  ...
	  break;
	    ...
	    if(mode >1)
	    {
	      if(getans2(b_lines, 0, "¬O§_½s¿è¥i¬Ý¨£¦W³æ¡H", 0, 2, 'n') == 'y')	      
	      {
	        char buf[PATHLEN];
	        if(dashf(fname))
	          sprintf(buf,"%s.vis",fname);
	...
	case 'R':
	  if (me.now < me.num)
	  {
	    ...
	    if(mode < 1)
	    {
	      pressanykey ("³o¬O¨p¤H­«¦a¡AµLªk¶i¤J¡I¦³»Ý­n½Ð¦VªO¥D¥Ó½Ð¡I");
	      me.page = 9999;
	      break;
	    }
...
AnnounceSelect ()
{
  ...
  brdcomplete ("¿ï¾ÜºëµØ°Ï¬ÝªO¡G", buf);
  if (*buf)
    strcpy (xboard, buf);
