...
uinfo_query(userec *u, int real, int unum)
{
  ...
  memcpy(&x, u, sizeof(userec));
  {
    ...
    ans = getans2(b_lines, 0, real ? "" : "½Ð¿ï¾Ü¡G", choose, real ? 6 : 3, 0);
  }
  ...
  case '1':       /*³o¸Ì¦³¼g¤J¨ì .PASSWDS  ©Ò¥H§ï¶V¤Ö¶V¦n..hialan.020729*/
...

    {
      ...
      buf[0] = getans2(i++, 0, "©Ê§O¡G", choose_sex, 8, u->sex + '1');
      if (buf[0] >= '1' && buf[0] <= '8')
        x.sex = buf[0] - '1';
    }
...
unsigned u_habit()
{
  ...
  register int i;
  int ch,myrow,mycol;
  const int nrow = NUMHABITS/2;  /* »Ý­n´X¦æ */
  ...
  move(b_lines,0);
  outs("\033[47;30m                                   ¡ö¡ô¡õ¡÷)²¾°Ê¿ï¶µ  Enter)§ó§ï³]©w  Q)Â÷¶}  \033[m\n");
  myrow=0; mycol=0;
  clrtobot(); move(3,2);
  do
  {
    ...
    move(b_lines-1,0);
    clrtoeol();
    prints("[1;33m»¡©ú¡G[m%s",habit_help_strings[myrow+mycol*nrow]);
...
showplans(char* uid)
{
  ...
  switch (getans2(b_lines, 0, "", choose_plans, 5, 0))
  {
    ...
    case '2':
    {
      ...
      if(n==0)
      {
        pressanykey("%s ¨S¦³¥ô¦óÃ±¦WÀÉ¡I", uid);
        break;
      }
      ...
      do
      {
        i = getans2(b_lines, 0, "½Ð¿ï¾ÜÃ±¦WÀÉ¡G", choose_tmp, n, 'q');;
...
*/    
  ...
  if (ans2 == 'e')
  {
    setutmpmode(mode);
    if (vedit(buf, NA) == -1)
      pressanykey("%s ¨S¦³§ó°Ê¡I", msg2);
    else
      pressanykey("%s §ó·s§¹²¦¡I", msg2);
  }
  else if (ans2 == 'd')
    unlink(buf);
  return 0;
...
int u_list()
{
  ...
  if (HAS_PERM(PERM_SEEULEVELS))
  {
    ...
    if (getans2(b_lines, 0, "Æ[¬Ý¡G", choose, 2, '1') != '2')
      u_list_special = 32;
  }
  if (HAS_PERM(PERM_CHATROOM) || HAS_PERM(PERM_SYSOP))
  {
    ...
    if (getans2(b_lines, 0, "Åã¥Ü¡G", choose, 2, '1') != '2')
      showrealname = 1;
  }
  u_list_CB(NULL);
...
userlevel()
{
  ...
  total=(tmp/1440) * 100;	//¤W¯¸®É¶¡ (¤Ñ) x 100
  total+=cuser.numlogins * 50;	//¤W¯¸¦¸¼Æ     x 50
  total+=cuser.numposts * 50;	//µoªí¤å³¹¦¸¼Æ x 50
  total+=cuser.receivemsg * 5;	//¦¬°T®§¦¸¼Æ    x 5
  total+=cuser.sendmsg * 5;	//µo°T®§¦¸¼Æ   x 5
  total+=cuser.bequery * 100;	//¤H®ð«ü¼Æ     x 100 
  
  total = sqrt(total);
  tmp = (int) total;

  for(i = -1; tmp; i++)	//¦]¬°³Ì«áÁÙ­n¦A¥[¤@¦¸:p
    tmp/=2;

  sprintf(buf, "¨Ï¥ÎªÌ %s µ¥¯Å¬O¡G%d", cuser.userid, i);
  pressanykey(buf);
