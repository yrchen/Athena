...
int setforward() /* Ptt , modify by wildcat*/
{
  ...
  getdata(b_lines-1,0,"½Ð¿é¤J«H½c¦Û°ÊÂà±Hªºemail¦a§}:", ip,41, DOECHO,ip);
  ...
  {
    if(getans2(b_lines,0,"½T©w¶}±Ò¦Û°ÊÂà«H¥\\¯à¡H",0,2,'y') != 'n' &&  (fp=fopen(buf,"w")) != NULL)
    {
      fprintf(fp,"%s",ip);
      fclose(fp);
      pressanykey("³]©w§¹¦¨¡I");
...
m_internet()
{
  ...
  {
    move(22, 0);
    pressanykey("¦¬«H¤H©Î¥DÃD¤£¥¿½T¡A½Ð­«·s¿ï¨ú«ü¥O¡I");
  }
  return 0;
}
  ...
  char *userid, *title;
{
  ...
  if (internet_mail)
  {
    ...
    switch (ch = getans2(3, 0, "½T©w­n±H¥X¶Ü? ", 0, 2, 'y'))
    {
    case 'n':
      outs("N\n«H¥ó¤w¨ú®ø");
      res = -2;
...
"A)¼W¥[      D)§R°£          M)½T»{±H«H¦W³æ    %sQ)¨ú®ø ¡H[M]",
...
mail_all()
{
   ...
   setutmpmode(SMAIL);
#if 0
   if(answer("¬O§_­nµ¹¿ú  Y)½T©w  N)¨ú®ø¡H") == 'y')
   {
     while(money <= 0)
     {
...
mailtitle(int tag)
{
  switch(tag)
  {
    ...
    case RC_FOOT:
      move(b_lines, 0);
      clrtoeol();
      prints("%s  «H¥ó¦Cªí  %s       =[]<>)¥DÃD¦¡¾\\Åª  ¡ö¡ô¡õ¡÷|PgUp|PgDn|Home|End)¾ÉÄý  h)»¡©ú \033[m",
              COLOR2, COLOR3);
      break;
  }
}
  ...
  char *direct;
{
  ...
  if (HAS_PERM(PERM_SYSOP) || !strcmp(fhdr->owner, cuser.userid))
  {
    ...
    {
      ent = 0;
      if (getans2(2,0,"«O¯d­ì§@ªÌ¦WºÙ¶Ü¡H",0,2,'y') != 'n') author = 1;
    }
  }
...
mail_justify(userec muser)
{
  ...
  if (valid_ident(muser.email) && !not_addr(muser.email))
  {
    ...
    {
      pressanykey("¿ù»~¡I¶}±ÒÀÉ®×¥¢±Ñ¡A½Ð³qª¾¯¸ªø¡I");
      return;
    }
    fprintf(fp, "%s\n", MagicKey);
    fclose(fp);

    sprintf(title, "%s To %s: [­«­n¡I½Ð¾\\Åª]", TAG_VALID, muser.userid);
