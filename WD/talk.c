...
my_talk(user_info *uin)
{
  ...
  char *choose_ques[4]={"yY)²á¤Ñ", "cC)¶H´Ñ", "dD)·t´Ñ", msg_choose_cancel};
  ...
  {
    switch(genbuf = getans2(2, 0, "§ä¥L ", choose_ques, 4, 'q'))
    {
      case 'y':		//²á¤Ñ
        uin->dark_turn=0;
        uin->turn = 0;
...
search_pickup(int num, int actor, pickup pklist[])
{
  ...
  namecomplete("½Ð¿é¤J¨Ï¥ÎªÌ©m¦W¡G", genbuf);
  ...
  char *uident;
{
  ...
  if (uident[0] > ' ')
  {
    ...
    if (getans2(2, 0,"Ãa¤H¶Ü¡H", 0, 2, 'n') != 'y')
    {
      pal.ftype |= M_PAL;

      if (strcmp(uident, cuser.userid) && getans2(2, 0, "¥[¤J¤W¯¸³qª¾¶Ü¡H", 0, 2, 'y')!= 'n')
      {
        PAL aloha;
  ...
  char *uident;
{
  ...
  sprintf(fpath, "½T©w²¾°£¦n¤Í %s¡H", uident);
...
change_talk_type()
{
  ...
  char ans = getans2(b_lines-1, 0,"±Æ§Ç¤è¦¡¡G",prompt, 6, pickup_way + '1');
...
{
  char buf[50];
  if(getans2(1, 0, "½Ð°Ý¬O§_­×§ï¬G¶m¡H", 0, 2, 'n') == 'y')
  {  
...
talk_switch()  /* Åã¥Ü¤Á´« */
{
  char *choose[6]={"sS)¦n¤Í´y­z",
  		   ...
  		   "yY)TTY",
  		   "iI)PID"};
  int ch = getans2(b_lines, 0, "¤Á´«¡G", choose, HAS_PERM(PERM_SYSOP) ? 6 : 1, '1');
...
KEY_TAB, change_talk_type,  0, "¤Á´«±Æ§Ç¤èªk¡C",0,
'N',	  talk_chusername,  PERM_BASIC, "­×§ï¼ÊºÙ¡C",0,
'M',	  talk_chmood,	    PERM_BASIC, "­×§ï¤ß±¡¡C",0,
'F',	  talk_chhome,	    PERM_BASIC, "­×§ï¬G¶m¡C",0,
'b',	  talk_broadcast,   PERM_PAGE, "¼s¼½¡C",0,
's',	  talk_switch,	    PERM_LOGINOK, "Åã¥Ü¦n¤Í´y­z¡C",0,
't',	  talk_ask, 	    PERM_PAGE, "²á¤Ñµ¥¬ÛÃö¥\\¯à¡C",0,
'w',	  talk_water,       PERM_PAGE, "¥á¤ô²y¡C",0,
'i',	  u_cloak, 	    PERM_CLOAK, "Áô§Î¡C",0,
'a',	  talk_editfriend,  PERM_LOGINOK, "¥[¤J/§R°£ ¦n¤Í¡C",0,
'd',	  talk_editfriend,  PERM_LOGINOK, "¥[¤J/§R°£ ¦n¤Í¡C",0,
'o',	  talk_friendlist,  PERM_LOGINOK, "½s¿è¦n¤Í¦W³æ¡C",0,
'f',	  talk_chfriend,    0, "¤Á´«Åã¥Ü¦n¤Í/¤@¯ëª¬ºA",0,
'q',	  talk_query, 	    0, "query ¤H",0,
'm',	  talk_sendmail,    PERM_BASIC, "±H«H",0,
'r',	  m_read, 	    PERM_BASIC, "Åª«H",0,
'l',	  t_bmw, 	    PERM_BASIC, "¤ô²y¦^ÅU",0,
'p',	  t_pager,          PERM_BASIC, "¤Á´«áà¾÷ª¬ºA¡C",0,
'H',	  talk_sysophide,   PERM_SYSOP, "¤Á´«¯¸ªøÁô¨­¡C¯¸ªø±M¥Î!!",0,
'D',	  talk_chuser,      PERM_SYSOP, "¼È®É¤Á´«¨Ï¥ÎªÌ¡C¯¸ªø±M¥Î!!",0,
'u',	  talk_edituser,    PERM_ACCOUNTS, "­×§ï¨Ï¥ÎªÌªº¸ê®Æ¡C¯¸ªø±M¥Î!!",0,
'K',	  talk_kick,	    PERM_SYSOP, "¯¸ªø½ð¤H¡C¯¸ªø±M¥Î!!",0,
0, NULL, 0, NULL,0};
...
pickup_user()
{
  ...
  while (1)
  {
    ...
    if (utmpshm->uptime > freshtime)
    {
      ...
      if (!actor)
      {
        if (getans2(b_lines, 0, "§AªºªB¤ÍÁÙ¨S¤W¯¸¡A­n¬Ý¬Ý¤@¯ëºô¤Í¶Ü¡H", 0, 2, 'y') != 'n')
        {
          cuser.uflag ^= FRIEND_FLAG;
          continue;
        }
	...
	COLOR3,
        ...
        "¼ÊºÙ (N)", 
        show_friend ? "¦n¤Í´y­z (s)" : show_board ? "¨Ï¥Î¬ÝªO" : "¬G¶m (F)",
...
t_idle()
{
  ...
  genbuf[0] = getans2(b_lines, 0,"²z¥Ñ¡G",reason,8,'0');
  if (genbuf[0] == 'q')
  {
    currutmp->mode = mode0;
    currstat = stat0;
      ...
      currutmp->destuid = 0;
  {
    ...
    do
    {
      move(b_lines - 1, 0);
      clrtoeol();
      sprintf(buf, "(Âê©w¿Ã¹õ)µo§b­ì¦]¡G%s", (currutmp->destuid != 6) ?
...
void talkreply()
{
  ...
  sprintf(genbuf, "§A·Q¸ò %s %s¶Ü¡H½Ð¿ï¾Ü(Y/N/A/B/C/D/E)[Y] ",
    page_requestor, !currutmp->turn ? "²á¤Ñ" :
    		    currutmp->dark_turn == 1 ? "·t´Ñ" :
    		    currutmp->dark_turn == 2 ? "¶H´Ñ" :
    		    currutmp->dark_turn == 3 ? "¤­¤l´Ñ" : "");
    		    
  getdata(0, 0, genbuf, buf, 4, LCECHO,0);

  if (uip->mode != PAGE) 
  {
     pressanykey("%s¤w°±¤î©I¥s", page_requestor);
     return;
  }
...
lockutmpmode(int unmode)
{
  ...
  {
   char buf[80];
   sprintf(buf,"©êºp¡A±z¤w¦³¨ä¥L½u¬Û¦Pªº ID ¥¿¦b %s",ModeTypeTable[unmode]);
   pressanykey(buf);
   return 1;
  }
  setutmpmode(unmode);
