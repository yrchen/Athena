...
//    if (getans("±z·Q§R°£¨ä¥L­«½Æªº login (Y/N)¶Ü¡H[Y] ") != 'n')
  ...
  else  /* guestªº¸Ü */
  {
    if (count_multi() > 32)
    {
      pressanykey("©êºp¡A¥Ø«e¤w¦³¤Ó¦h guest¡A½Ðµy«á¦A¸Õ¡I");
      oflush();
      exit(1);
    }
  }
...
over_load()
{
  ...
  {
    pressanykey("¥Ø«e¨t²Î­t²ü %f ¹L°ª¡A½Ðµy«á¦A¨Ó¡I",cpu_load[0]);
    return 1;
  }  
  return 0;
}
...
*/
  ...
  {
    pressanykey("¥Ø«e¯¸¤W¤H¼Æ¤w¹F¤W­­¡A½Ð±zµy«á¦A¨Ó¡I");
    oflush();
    sleep(1);
    exit(1);
  }
  ...
  while (1)
  {
    ...
    {
      pressanykey("¿ù»~¤Ó¦h¦¸¡AÙTÙT +_+");
      oflush();
      exit(1);
    }
    getdata(b_lines - 2, 40, "½Ð¿é¤J±b¸¹¡G",uid, IDLEN + 1 , DOECHO,0);
...
//      new_register(0);
    ...
    {
      getdata(b_lines - 2, 40, "½Ð¿é¤J±K½X¡G", passbuf, PASSLEN, NOECHO,0);
      passbuf[8] = '\0';
