...
listtitle(int tag)
{
  switch(tag)
  {
    case RC_FULL:
    {
      ...
      if (currstat != LISTMAIN)
      {
        prints("%s\033[30m◤\033[37m水球◢%s名單%s\033[37m◣說明\033[36;40m◣\033[m    a)新增  c)修改  d)刪除  m)移動  i)引入名單  s)群組寄信\n", COLOR1, COLOR3, COLOR1);
        prints("%s  編號 模式 日 期 名  稱        描        述                                   \033[0m", COLOR3);
      }
      else
      {
        prints("%s\033[30m◤\033[37m水球◢%s名單%s\033[37m◣說明\033[36;40m◣\033[m                            a)新增  c)修改  d)刪除  m)移動\n", COLOR1, COLOR3, COLOR1);
    ...
    case RC_FOOT:
      move(b_lines, 0);
      clrtoeol();
      prints("%s  群組名單  %s %65s \033[m", COLOR2, COLOR3, "←↑↓→|PgUp|PgDn|Home|End)導覽  h)說明");
      break;
  }
}
