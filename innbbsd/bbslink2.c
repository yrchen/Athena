char *COMMENT =
"\
[1;37m¢¨    ¢© ¢i¢i¢i¢© [0;34m¢c¢c¢c¢c¢c¢c¢c¢c¢c¢c¢c¢c¢c¢c¢c¢c¢c¢c¢c[m\n\
[1;36m¢i¢¨¢©¢i  ¢i  ¢i     [36m­·[37m»P[33m¹Ð®J[37mªº¹ï¸Ü [31mBBS [30m£»[1;35m wdbbs.net[m\n\
[1;34m¢ª¢«¢ª¢« ¢i¢i¢i¢« [0;34m¢h¢h¢h¢h¢h¢h¢h¢h¢h¢h¢h¢h¢h¢h¢h¢h¢h¢h¢h[m";
  ...
  char *board, *filename, *from, *subject;
{
  ...
  if (strncmp(result, "post to ", 8) == 0)
  {
    ...
    if (isfile(filepath))
    {
      f_mv(filepath, fileglue("%s.cancel", filepath));
    }
    FN = fopen(filepath, "w");
...
*/
  ...
  fprintf(stderr, "¥»µ{¦¡­n¥¿±`°õ¦æ¥²¶·±N¥H¤UÀÉ®×¸m©ó %s/innd ¤U:\n", BBSHOME);
  fprintf(stderr, "bbsname.bbs   ³]©w¶Q¯¸ªº BBS ID (½Ð¾¨¶qÂ²µu)\n");
...
struct fileheader
{
  char filename[33];	        /* M.9876543210.A 	33 bytes*/
  char savemode;                /* file save mode 	 1 bytes*/
  char owner[14];		/* uid[.] 		14 bytes*/
  char date[6];                 /* [02/02] or space(5)   6 bytes*/
  char title[73];		/* title		73 bytes*/
  char filemode;              /* must be last field @ boards.c 1 bytes*/
};
...
/*
woju
    if (!f_mv(fileglue("%s/out.bntp", INNDHOME), OUTING) && bbslink_get_lock(bbslink_lockfile))
*/
    if (strcpy(fname, ".DIR"))
    {
