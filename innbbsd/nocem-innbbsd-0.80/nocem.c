...
#ifdef  GPG
int
run_gpg(char *cmd, FILE ** in, FILE ** out)
{
  int pin[2], pout[2], child_pid;
  char GPGPATH[80];

  sprintf(GPGPATH, "%s/.gnupg", BBSHOME);
  setenv("GPGPATH", GPGPATH, 1);
...
verify_buffer(char *buf)
{
  ...
  setenv("PGPPASSFD", "0", 1);
  run_gpg("/usr/local/bin/gpg --no-secmem-warning --verify",
    &pgpin, &pgpout);
  if (pgpin && pgpout)
  {
    fprintf(pgpin, "%s", buf);
