...
/* process post write */
int
bbspost_write_post(int fh, char *board, char *filename)
{
  char *fptr, *ptr;
  FILE *fhfd = fdopen(fh, "w");
  char mynick[128], myaddr[128], newfrom[256];
  ...
  str_from(FROM, myaddr, mynick);  
  if( (*mynick) != '\0')
    sprintf(newfrom, "%s (%s)", myaddr, mynick);
  else
    strlcpy(newfrom, myaddr, sizeof(newfrom));

  fprintf(fhfd, "發信人: %.60s, 看板: %s\n", newfrom, board);    
  str_decode(SUBJECT);  // Dopin: by PaulLiu.bbs@processor.tfcis.org 
...
void
cancelpost(fileheader *fhdr, char *boardname)
{
  int fd;
  ...
  if ((fd = open(fpath, O_RDONLY)) >= 0) 
  {
    ...
    rec_add(fn2, &postfile, sizeof(postfile));

    if ((bid = getbnum(boardname)) > 0)
      touchbtotal(bid);
  }
  else
  ...
  while (read(fd, &header, size) == size)
  {
    if (strcmp(file, header.filename) == 0)
    {
       if ((header.filemode & FILE_MARKED)
          ...
          break;
       delete_record(dirname, sizeof(fileheader), lseek(fd, 0, SEEK_CUR) / size);
