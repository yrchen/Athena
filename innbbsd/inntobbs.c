...
"Mime-Version", MIME_VERSION_H,
"Content-Type", MIME_CONTENTTYPE_H,
"Content-Transfer-Encoding", MIME_CONTENTENCODE_H,
};
...
nodelist_t *nl;
{
     ...
     if (nl->exclusion && *nl->exclusion) {
        char *exclude, *ptr;
	for (exclude = nl->exclusion, ptr = strchr(exclude, (int) ','); 
	     exclude && *exclude; ptr = strchr(exclude, (int) ',')) 
	{
	     if (ptr) *ptr = '\0';
...
ClientType *client;
{
	...
	*ptr = '\0';
	   ...
	   break;
        ...
        /* Dopin: For RFC 822 by PaulLiu.bbs@processor.tfcis.org */
        if(front[0] == ' ' || front[0] == '\t') {
           if(value >= 0 && value < LASTHEADER && HEADER[value] != NULL)
              for(i = 0 ; &(front[i]) > HEADER[value] ; i--)
                 if(front[i] == '\0' || front[i] == '\r' || front[i] == '\n')
                    front[i] = ' ';
                                                                                
           for(i = 0 ; front[i] != '\0' ; i++)
              if(front[i] == '\r' || front[i] == '\n') {
                 front[i] = '\0';
              }
